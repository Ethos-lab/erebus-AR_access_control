...
{
    [SerializeField] private Erebus.AR.ErebusARManager erebusARManager;
    ...
    private void Update()
    {
        ...
        if (!blockUI)
        {
            ...
            //var curRot = curFurniture.transform.rotation.eulerAngles;
            //var angle = -999f;
            if (erebusARManager.FunctionProvider.Raycast(screenCenter, hitRes, TrackableType.PlaneWithinPolygon))
            {
                var hitObj = hitRes[0];
                var hitPos = hitObj.pose.position;
                var hitRot = hitObj.pose.rotation;
                curFurniture.transform.position = hitPos;
                //curFurniture.transform.rotation = Quaternion.Euler(curRot.x, curRot.y, hitRot.z);
                //curRot.z = hitRot.z;
