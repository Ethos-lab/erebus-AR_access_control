...
namespace Erebus
{
    namespace AR
    {
        public class ErebusARManager : MonoBehaviour
        {
            ...
            [SerializeField] private float gpsUpdateInterval = 5.0f; //Change this to a smaller value (1.0f) for AR Navigation; More frequent update
            ...
            private void Start()
            {
                ...
                appDataController.Initialize(serverIpAddress, serverPort, gpsUpdateInterval, this, this);
        ...
        namespace ARFunctionProvider
        {
            public class FunctionProvider
            {
                ...
                private byte[] GetWhitelistedRawPixels()
                {
                    ...
                    //Flip vertically (Only for AR Remote Maintenance : If the phone is Landscape Left) 
                    if (flipCameraImage)
        ...
        namespace Data
        {
            ...
            public class DataEntryPoint
            {
                ...
                public void Initialize(string serverIpAddress, string serverPort, float gpsUpdateInterval, ErebusARManager mainManager, MonoBehaviour monoBehaviourObj)
                {
        ...
        namespace Networking
        {
            public class NetworkCommunicator
            {
                ...
                //public byte[] SendImgBytes(byte[] imgBytes, string targetTag = "")
                //{
                ...
                //            //stopwatch.Restart();
                //            ProcessRequestForm rqst = new ProcessRequestForm()
                //            {
                //                Img = imgBytes,
                //                TargetTag = targetTag
                //            };
                //            var serializedBytes = MessagePackSerializer.Serialize(rqst);
                //            socket.SendFrame(serializedBytes);
                //            if (socket.TryReceiveFrameBytes(timeout, out byte[] resultBytes))
                //            {
                //                //stopwatch.Stop();
                //                //Debug.Log($"Network : {stopwatch.Elapsed.TotalMilliseconds}");
                //                //stopwatch.Restart();
                    ...
                    using (var socket = new RequestSocket($"tcp://{ipAddr}:{portNum}"))
                    {
                        try
                        {
                            ...
                            if (socket.TryReceiveFrameBytes(timeout, out byte[] compressedResultBytes))
                            {
                                ...
                                //var decompressedResultBytes = compressedResultBytes;
                                var decompressedResultBytes = Decompress(compressedResultBytes);
                                stopwatch.Stop();
                                Debug.Log($"Decompress : {stopwatch.Elapsed.TotalMilliseconds}");
                                stopwatch.Restart();

                                //Null data
                                if (decompressedResultBytes.Length <= 1)
                                    return null;

                                var msgPackDetectionResult = MessagePackSerializer.Deserialize<MsgPackDetectionResult>(decompressedResultBytes);
                                stopwatch.Stop();
                                Debug.Log($"Deserialize : {stopwatch.Elapsed.TotalMilliseconds}");
                                stopwatch.Restart();
                                ...
                                stopwatch.Stop();
                                Debug.Log($"DetectionResult : {stopwatch.Elapsed.TotalMilliseconds}");
