...
public class CameraModule : MonoBehaviour
{
    ...
    public string requestedCameraName = "Camera 1"; //Front camera
    ...
    private IEnumerator _Initialize()
    {
        ...
        if (!String.IsNullOrEmpty(requestedCameraName))
        {
            //int requestedDeviceIndex = -1;
            //if (Int32.TryParse(requestedCameraName, out requestedDeviceIndex))
            //{
            ...
            {
                if (devices[cameraIndex].name == requestedCameraName)
                {
            ...
            if (webCamTexture == null)
                Debug.Log("Cannot find camera device " + requestedCameraName + ".");
        }
    ...
    public void OnSaveButtonClick(bool isFaceId = false)
    {
        Utils.webCamTextureToMat(webCamTexture, rgbaMat, colors);

        //Save mat to rotated bytes
        Mat saveMat = rgbaMat.clone();
        if (!isFaceId)
            Core.rotate(saveMat, saveMat, Core.ROTATE_90_COUNTERCLOCKWISE);
        ...
        Imgproc.resize(saveMat, resizedCamMat, new Size(renderCanvasSize.x, renderCanvasSize.y));
        ...
        Imgproc.cvtColor(RoiSaveMat, RoiSaveMat, Imgproc.COLOR_GRAY2BGR);

        ////To change the channel to 3 channels
        //Imgcodecs.imwrite(Path.Combine(Application.persistentDataPath, "tmp.jpg"), RoiSaveMat);
    ...
    public Mat RoiSaveMat { get; set; } = null;
    ...
    public Mat ConvertImageStringToMat(string faceDataString)
    {
        ...
        var readMat = Imgcodecs.imdecode(readMatBytes, Imgcodecs.IMREAD_COLOR);
        return readMat;
    }
    public void SaveToImageFromStringBytes(string path, string faceDataString)
    {
        var readMat = ConvertImageStringToMat(faceDataString);
    ...
    /// </summary>
    public void OnPauseButtonClick(bool isFaceId = false)
    {
        if (hasInitDone)
        {
            IsPaused = true;
            webCamTexture.Pause();
            OnSaveButtonClick(isFaceId);
        }
    }
