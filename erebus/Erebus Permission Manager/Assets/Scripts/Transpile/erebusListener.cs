namespace erebus.Core.Transpile
{
    ...
    public class erebusListener : erebusBaseListener
    {
        ...
        // flag for overriding default false policy
        ...
        public override void EnterAction([NotNull] erebusParser.ActionContext context)
        {
            AddTemplateDepth();
            if (context.POLICY().GetText() == "Allow")
            {
        ...
        private object ResolveComparisonExpr(erebusParser.ComparisonExprContext comparisonExprContext)
        {
            ...
            else if ( comparisonExprContext.CONTAINS() != null)
            {
                ...
                var op = comparisonExprContext.CONTAINS().GetText();

                Console.WriteLine($"Comparison operation : op = {op} ");
                if (op.Contains("matches"))
                {
                    return list + ".Matches(" + elem + ")";
                }
                else if (op.Contains("within"))
                {
                    return list + ".Within(" + elem + ")";
                }
                else
                {
